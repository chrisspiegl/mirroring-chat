...
const pnotice = require('pushnotice')(`${config.slug}:router:authentication`, {env: config.env, chat: config.pushnotice.chat, debug: true, disabled: config.pushnotice.disabled})
...
const login = () => {
  ...
  router.get('/:provider/unlink', async (req, res) => {
    ...
    resUserProviderDestroy = await userProvider.destroy()
